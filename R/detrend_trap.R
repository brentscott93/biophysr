...
#'
#' @examples
detrend_trap <- function(x, delta_t){
  
  #get length of data record
  data_length <- length(x)
  
  #find number of traces concactenated together
  traces_sampled <- data_length/delta_t
  
  #build dataframe of indices
  bp_df <-  data.frame(start = seq(1, data_length, by = delta_t), 
                      stop = tail(seq(0, data_length, by = delta_t), -1))
